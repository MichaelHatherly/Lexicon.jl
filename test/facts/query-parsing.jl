facts("Query parsing.") do
    ...
    context("Methods.") do
        ...
        @fact @query(f(1)) => Query((which(f, (Any,)),), 0)
        @fact @query(g(1)) => Query((which(g, (Any,)),), 0)

        @fact @query(f(), 1) => Query((which(f, ()),), 1)
        @fact @query(g(), 1) => Query((which(g, ()),), 1)

        @fact @query(f(1), 1) => Query((which(f, (Any,)),), 1)
        @fact @query(g(1), 1) => Query((which(g, (Any,)),), 1)


        @fact @query(A.f()) => Query((which(A.f, ()),), A, 0)
        @fact @query(A.g()) => Query((which(A.g, ()),), A, 0)

        @fact @query(A.f(1)) => Query((which(A.f, (Any,)),), A, 0)
        @fact @query(A.g(1)) => Query((which(A.g, (Any,)),), A, 0)

        @fact @query(A.f(), 1) => Query((which(A.f, ()),), A, 1)
        @fact @query(A.g(), 1) => Query((which(A.g, ()),), A, 1)

        @fact @query(A.f(1), 1) => Query((which(A.f, (Any,)),), A, 1)
        @fact @query(A.g(1), 1) => Query((which(A.g, (Any,)),), A, 1)


        @fact @query(A.B.f()) => Query((which(A.B.f, ()),), A.B, 0)
        @fact @query(A.B.g()) => Query((which(A.B.g, ()),), A.B, 0)

        @fact @query(A.B.f(1)) => Query((which(A.B.f, (Any,)),), A.B, 0)
        @fact @query(A.B.g(1)) => Query((which(A.B.g, (Any,)),), A.B, 0)

        @fact @query(A.B.f(), 1) => Query((which(A.B.f, ()),), A.B, 1)
        @fact @query(A.B.g(), 1) => Query((which(A.B.g, ()),), A.B, 1)

        @fact @query(A.B.f(1), 1) => Query((which(A.B.f, (Any,)),), A.B, 1)
        @fact @query(A.B.g(1), 1) => Query((which(A.B.g, (Any,)),), A.B, 1)
